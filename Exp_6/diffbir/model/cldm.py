...
from .vae import AutoencoderKL, AutoencoderKLResi
...
class ControlLDM(nn.Module):
    ...
    def __init__(
        ...
        self.vae = AutoencoderKLResi(**vae_cfg)
    ...
    def prepare_condition(
        ...
        # )
        c_txt = self.clip.encode(txt)          # 【CFW新增】
        c_img, enc_fea = self.vae_encode(
            cond_img * 2 - 1,   
            sample=False,
            tiled=tiled,
            tile_size=tile_size,
        ...
        c_rgb = cond_RGB  
        return dict(c_txt=c_txt, c_img=c_img, c_rgb=c_rgb, enc_fea=enc_fea)
