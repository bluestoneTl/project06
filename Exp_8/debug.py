...
#  python debug.py --config configs/train/train_stage2.yaml --output_dir results/3.20_vae_inference/debug4
def main(args) -> None:
    # Setup accelerator:
    ...
    while global_step < max_steps:
        ...
        for batch in loader:
            ...
            loss = loss + vae_loss * 0.1   # 0.2-0.05     0.008-0.006
            ...
            if accelerator.is_main_process:
                ...
                for batch_idx in range(reconstructed.shape[0]):
                    # 将张量移动到CPU并分离计算图
                    recon_img = reconstructed[batch_idx].cpu().detach()
                    
                    # 若图像数据范围是 [-1, 1]，归一化到 [0, 1]
                    if recon_img.min() < 0:
                        recon_img = (recon_img + 1) / 2
                    
                    # 转换为numpy数组并调整通道顺序
                    image = recon_img.numpy().transpose(1, 2, 0)  # CHW -> HWC
                    ...
                    save_path = os.path.join(
                        args.output_dir, 
                        f"reconstructed_step_{global_step}_idx_{batch_idx}.png"
                    )
